...
The VLT role requires an SSH connection for connectivity to your Dell EMC Networking device. You can use any of the built-in OS connection variables or the *provider* dictionary.
...
- Role is abstracted using the *ansible_net_os_name* variable  that can take dellos9 and dellso10 values  
...
| ``delay_restore``     | integer, default=90        | Configures the delay in bringing up VLT ports after reload or peer-link restoration | dellos9 |
| ``delay_restore_abort_threshold``     | integer, default=60        | Configures the wait interval for VLT delay-restore timer to abort | dellos9 |
...
| ``remote_mac.address``     | string        | Configures the remote MAC address for static VLT proxy gateway  | dellos9 |
| ``remote_mac.exclude_vlan_range``     | string        | Configures the exclude VLAN for static VLT proxy gateway  | dellos9 |
...
| ``static.proxy_static_state``        | string: absent,present     | Deletes the static vlt proxy gateway when set to absent | dellos9 |
...
| ``state``        | string, choices: absent,present     | Removes the VLT instance if set to absent | dellos9, dellos10 |
...
Ansible Dell EMC Networking OS roles require connection information to establish communication with the nodes in your inventory. This information can exist in the Ansible *group_vars* or *host_vars* directories, or in the playbook itself.
...
| ``username`` | no       |            | Specifies the username to use to authenticate the CLI login for connection to the remote device; if value is unspecified, the ANSIBLE_NET_USERNAME environment variable value is used |
...
This example uses the *dellos-vlt* role to setup a VLT-domain. It creates a *hosts* file with the switch details and corresponding variables.The hosts file should define the *ansible_net_os_name* variable with corresponding Dell EMC networking OS name.
